...
export const createUserWithEmailAndPasswordHandler = async (email: string, password: string) => {
  const credentials = await createUserWithEmailAndPassword(auth, email, password);
  console.log(credentials);
  return credentials;
};

export const createUserDoc = async (userAuthId: string, data: firebaseUsersData) => {
  try {
...
export const signInWithGoogle = async (navigation: NavigateFunction) => {
  try {
    ...
    dataToSet && setToSessionStorage({ ...dataToSet, id: credentials.user.uid });
    navigation('/');
  } catch (e) {
    console.log('error in getting info by user id: ', e);
  }
...
export const signUpWithEmailAndPasswordHandler = async (email: string, password: string) => {
  try {
    const credentials = await createUserWithEmailAndPassword(auth, email, password);
    const dataToSet = await getDataByUserId(credentials.user.uid);
    dataToSet && setToSessionStorage({ ...dataToSet, id: credentials.user.uid });
